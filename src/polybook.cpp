...
{
    enabled = false;
    if (bookfile.empty() || bookfile == "<empty>")
        return;
...
Move PolyBook::probe(Position& pos, bool bestBookMove)
{
    if (!enabled)
        return MOVE_NONE;
    ...
    int idx = bestBookMove || n == 1 ? index_best : index_rand;
    ...
    if (n > 1)
    {
        idx = idx == index_first ? index_first + 1 : index_first;
